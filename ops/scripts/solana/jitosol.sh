#!/usr/bin/env bash
# jitoSOL — hub leg: Solana, locking mode (Jito staked SOL, standard SPL —
# yield accrues in price, so locked amounts stay 1:1 with minted reps).
# Hub legs deploy first; the Hydration spoke leg comes later via
# scripts/hydration/jitosol.sh.
#
...
#
#   scripts/solana/jitosol.sh preflight   # no txs: toolchain, payer balance, CLI patches
#   scripts/solana/jitosol.sh init        # no txs: NTT_COMMIT, overrides.json, deployment.json
#   scripts/solana/jitosol.sh keys        # no txs: generate program-id keypair
#   scripts/solana/jitosol.sh deploy      # TX Solana: anchor build + program deploy (locking jitoSOL)
#   scripts/solana/jitosol.sh push        # TX Solana: register Hydration peer + limits (after spoke leg)
#
#   scripts/solana/jitosol.sh status      # local vs on-chain drift

set -euo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/_lib.sh"

DEPLOYMENT="$HYD_ROOT/tokens/jitosol/deployment.json"
KEYS_DIR="$HYD_ROOT/tokens/jitosol/keys"              # gitignored — never commit keypairs
PROGRAM_KEY="$KEYS_DIR/program.json"
JITOSOL=J1toso1uCk3RLmjorhTtrVwY9HJ7X8V9yYac6Y7kGCPn  # canonical jitoSOL mint, 9 decimals
...
cmd_preflight() {
  ...
  echo "jitoSOL mint decimals: $(curl -sS "$SOLANA_RPC" -X POST -H 'Content-Type: application/json' \
    -d '{"jsonrpc":"2.0","id":1,"method":"getTokenSupply","params":["'$JITOSOL'"]}' \
    | jq -r .result.value.decimals) (expect 9)"
...
cmd_deploy() {
  ...
  (cd "$NTT_SRC" && ntt add-chain Solana --latest --mode locking \
    --token $JITOSOL --payer "$SOLANA_PAYER_KEYPAIR" --program-key "$PROGRAM_KEY" \
...
# Hydration-side half runs from scripts/hydration/jitosol.sh push with its own key.
cmd_push() {
  payer
  cmd_status || true
