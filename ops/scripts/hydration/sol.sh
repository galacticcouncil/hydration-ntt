#!/usr/bin/env bash
# SOL — spoke leg: Hydration, burning mode. PRECOMPILE VARIANT (preferred):
...
#
# Run only AFTER the Solana hub leg (scripts/solana/sol.sh).
# Signs with HYDRATION_PRIVATE_KEY only (whitelisted Hydration deployer).
#
#   scripts/hydration/sol.sh preflight   # no txs: hub leg present, balance, whitelist note
#   scripts/hydration/sol.sh asset <id>  # no txs: derive+verify precompile ref from asset id
#   scripts/hydration/sol.sh deploy      # TX: manager+transceiver (burning), --skip-verify
#                                       #   (verify post-hoc via scripts/hydration/_verify.sh)
#   scripts/hydration/sol.sh limits      # no txs: write rate limits into deployment.json
#   scripts/hydration/sol.sh push        # TX Hydration: register Solana peer + limits
#
#   scripts/hydration/sol.sh minter      # no txs: print set_ntt_minter governance call
#   scripts/hydration/sol.sh status      # local vs on-chain drift
#
# Cross-link is two-sided: after 'push' here, also run scripts/solana/sol.sh
...
TOKEN_ADDR_FILE="$HYD_ROOT/tokens/sol/hydration-token.addr"

# 24h NTT rate limits, whole tokens. Solana side is 9-dec wSOL; Hydration
# side uses the runtime asset's decimals (read live from the precompile).
LIMIT_SOLANA_OUT="${LIMIT_SOLANA_OUT:-10000}"   # ≈ $725k @ $72/SOL (2026-07)
LIMIT_SOLANA_IN="${LIMIT_SOLANA_IN:-10000}"
...
cmd_preflight() {
  ...
  check_cli
  [ -n "$(hub_manager)" ] \
    || { echo "ERROR: no Solana manager in $DEPLOYMENT — run scripts/solana/sol.sh first"; exit 1; }
...
cmd_asset() {
  local id="${1:?usage: sol.sh asset <asset-id>}"
  need_tools cast jq
  [ ! -s "$TOKEN_ADDR_FILE" ] \
  ...
  confirm "Use this asset as the SOL representation?"
...
cmd_limits() {
  ...
  echo "Hydration asset decimals: $dec (Solana wSOL: 9)"
  local sfrac hfrac
  sfrac=$(printf '%09d' 0)
...
# scripts/solana/sol.sh push (the Solana payer).
cmd_push() {
  use_key HYDRATION_PRIVATE_KEY
  cmd_status || true
  ...
  cmd_status
  echo "Remember: run scripts/solana/sol.sh push for the Solana-side half."
}
