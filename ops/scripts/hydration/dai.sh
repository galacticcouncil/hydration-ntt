#!/usr/bin/env bash
# DAI — spoke leg: Hydration, burning mode. PRECOMPILE VARIANT (preferred):
...
#
# Run only AFTER the Ethereum hub leg (scripts/ethereum/dai.sh).
# Signs with HYDRATION_PRIVATE_KEY only (whitelisted Hydration deployer).
#
#   scripts/hydration/dai.sh preflight   # no txs: hub leg present, balance, whitelist note
#   scripts/hydration/dai.sh asset <id>  # no txs: derive+verify precompile ref from asset id
#   scripts/hydration/dai.sh deploy      # TX: manager+transceiver (burning), --skip-verify
#                                       #   (verify post-hoc via scripts/hydration/_verify.sh)
#   scripts/hydration/dai.sh limits      # no txs: write rate limits into deployment.json
#   scripts/hydration/dai.sh push        # TX Hydration: register Ethereum peer + limits
#
#   scripts/hydration/dai.sh minter      # no txs: print set_ntt_minter governance call
#   scripts/hydration/dai.sh status      # local vs on-chain drift
#
# Cross-link is two-sided: after 'push' here, also run scripts/ethereum/dai.sh
...
TOKEN_ADDR_FILE="$HYD_ROOT/tokens/dai/hydration-token.addr"

# 24h NTT rate limits, whole tokens. Ethereum side is 18-dec DAI; Hydration
...
cmd_preflight() {
  ...
  check_cli
  [ -n "$(hub_manager)" ] \
    || { echo "ERROR: no Ethereum manager in $DEPLOYMENT — run scripts/ethereum/dai.sh first"; exit 1; }
...
cmd_asset() {
  local id="${1:?usage: dai.sh asset <asset-id>}"
  need_tools cast jq
  [ ! -s "$TOKEN_ADDR_FILE" ] \
  ...
  confirm "Use this asset as the DAI representation?"
...
cmd_limits() {
  ...
  echo "Hydration asset decimals: $dec (Ethereum DAI: 18)"
  local efrac hfrac
  efrac=$(printf '%018d' 0)
...
# scripts/ethereum/dai.sh push (its own key).
cmd_push() {
  use_key HYDRATION_PRIVATE_KEY
  cmd_status || true
  ...
  cmd_status
  echo "Remember: run scripts/ethereum/dai.sh push for the Ethereum-side half."
}
