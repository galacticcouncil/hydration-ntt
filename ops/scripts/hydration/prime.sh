#!/usr/bin/env bash
# PRIME — spoke leg: Hydration, burning mode. PRECOMPILE VARIANT (preferred):
...
#
# Run only AFTER the Solana hub leg (scripts/solana/prime.sh).
# Signs with HYDRATION_PRIVATE_KEY only (whitelisted Hydration deployer).
#
#   scripts/hydration/prime.sh preflight   # no txs: hub leg present, balance, whitelist note
#   scripts/hydration/prime.sh asset <id>  # no txs: derive+verify precompile ref from asset id
#   scripts/hydration/prime.sh deploy      # TX: manager+transceiver (burning), --skip-verify
#                                         #   (verify post-hoc via scripts/hydration/_verify.sh)
#   scripts/hydration/prime.sh limits      # no txs: write rate limits into deployment.json
#   scripts/hydration/prime.sh push        # TX Hydration: register Solana peer + limits
#
#   scripts/hydration/prime.sh minter      # no txs: print set_ntt_minter governance call
#   scripts/hydration/prime.sh status      # local vs on-chain drift
#
# Cross-link is two-sided: after 'push' here, also run scripts/solana/prime.sh
...
TOKEN_ADDR_FILE="$HYD_ROOT/tokens/prime/hydration-token.addr"

# 24h NTT rate limits, whole tokens. Solana side is 6-dec PRIME; Hydration
# side uses the runtime asset's decimals (read live from the precompile).
LIMIT_SOLANA_OUT="${LIMIT_SOLANA_OUT:-100000}"   # not a stable — check spot and size before 'limits'
LIMIT_SOLANA_IN="${LIMIT_SOLANA_IN:-100000}"
...
cmd_preflight() {
  ...
  check_cli
  [ -n "$(hub_manager)" ] \
    || { echo "ERROR: no Solana manager in $DEPLOYMENT — run scripts/solana/prime.sh first"; exit 1; }
...
cmd_asset() {
  local id="${1:?usage: prime.sh asset <asset-id>}"
  need_tools cast jq
  [ ! -s "$TOKEN_ADDR_FILE" ] \
  ...
  confirm "Use this asset as the PRIME representation?"
...
cmd_limits() {
  ...
  echo "Hydration asset decimals: $dec (Solana PRIME: 6)"
  local sfrac hfrac
  sfrac=$(printf '%06d' 0)
...
# scripts/solana/prime.sh push (the Solana payer).
cmd_push() {
  use_key HYDRATION_PRIVATE_KEY
  cmd_status || true
  ...
  cmd_status
  echo "Remember: run scripts/solana/prime.sh push for the Solana-side half."
}
