#!/usr/bin/env bash
# USDT — spoke leg: Hydration, burning mode. PRECOMPILE VARIANT (preferred):
...
#
# Run only AFTER the Ethereum hub leg (scripts/ethereum/usdt.sh).
# Signs with HYDRATION_PRIVATE_KEY only (whitelisted Hydration deployer).
#
#   scripts/hydration/usdt.sh preflight   # no txs: hub leg present, balance, whitelist note
#   scripts/hydration/usdt.sh asset <id>  # no txs: derive+verify precompile ref from asset id
#   scripts/hydration/usdt.sh deploy      # TX: manager+transceiver (burning), --skip-verify
#                                        #   (verify post-hoc via scripts/hydration/_verify.sh)
#   scripts/hydration/usdt.sh limits      # no txs: write rate limits into deployment.json
#   scripts/hydration/usdt.sh push        # TX Hydration: register Ethereum peer + limits
#
#   scripts/hydration/usdt.sh minter      # no txs: print set_ntt_minter governance call
#   scripts/hydration/usdt.sh status      # local vs on-chain drift
#
# Cross-link is two-sided: after 'push' here, also run scripts/ethereum/usdt.sh
...
TOKEN_ADDR_FILE="$HYD_ROOT/tokens/usdt/hydration-token.addr"

# 24h NTT rate limits, whole tokens. Ethereum side is 6-dec USDT; Hydration
...
cmd_preflight() {
  ...
  check_cli
  [ -n "$(hub_manager)" ] \
    || { echo "ERROR: no Ethereum manager in $DEPLOYMENT — run scripts/ethereum/usdt.sh first"; exit 1; }
...
cmd_asset() {
  local id="${1:?usage: usdt.sh asset <asset-id>}"
  need_tools cast jq
  [ ! -s "$TOKEN_ADDR_FILE" ] \
  ...
  confirm "Use this asset as the USDT representation?"
...
cmd_limits() {
  ...
  echo "Hydration asset decimals: $dec (Ethereum USDT: 6)"
  local efrac hfrac
  efrac=$(printf '%06d' 0)
...
# scripts/ethereum/usdt.sh push (its own key).
cmd_push() {
  use_key HYDRATION_PRIVATE_KEY
  cmd_status || true
  ...
  cmd_status
  echo "Remember: run scripts/ethereum/usdt.sh push for the Ethereum-side half."
}
