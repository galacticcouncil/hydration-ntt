#!/usr/bin/env bash
# sUSDS — spoke leg: Hydration, burning mode. PRECOMPILE VARIANT (preferred):
...
#
# Run only AFTER the Ethereum hub leg (scripts/ethereum/susds.sh).
# Signs with HYDRATION_PRIVATE_KEY only (whitelisted Hydration deployer).
#
#   scripts/hydration/susds.sh preflight   # no txs: hub leg present, balance, whitelist note
#   scripts/hydration/susds.sh asset <id>  # no txs: derive+verify precompile ref from asset id
#   scripts/hydration/susds.sh deploy      # TX: manager+transceiver (burning), --skip-verify
#                                         #   (verify post-hoc via scripts/hydration/_verify.sh)
#   scripts/hydration/susds.sh limits      # no txs: write rate limits into deployment.json
#   scripts/hydration/susds.sh push        # TX Hydration: register Ethereum peer + limits
#
#   scripts/hydration/susds.sh minter      # no txs: print set_ntt_minter governance call
#   scripts/hydration/susds.sh status      # local vs on-chain drift
#
# Cross-link is two-sided: after 'push' here, also run scripts/ethereum/susds.sh
...
TOKEN_ADDR_FILE="$HYD_ROOT/tokens/susds/hydration-token.addr"

# 24h NTT rate limits, whole tokens. Ethereum side is 18-dec sUSDS; Hydration
# side uses the runtime asset's decimals (read live from the precompile).
LIMIT_ETHEREUM_OUT="${LIMIT_ETHEREUM_OUT:-100000}"
LIMIT_ETHEREUM_IN="${LIMIT_ETHEREUM_IN:-100000}"
...
cmd_preflight() {
  ...
  check_cli
  [ -n "$(hub_manager)" ] \
    || { echo "ERROR: no Ethereum manager in $DEPLOYMENT — run scripts/ethereum/susds.sh first"; exit 1; }
...
cmd_asset() {
  local id="${1:?usage: susds.sh asset <asset-id>}"
  need_tools cast jq
  [ ! -s "$TOKEN_ADDR_FILE" ] \
  ...
  confirm "Use this asset as the sUSDS representation?"
...
cmd_limits() {
  ...
  echo "Hydration asset decimals: $dec (Ethereum sUSDS: 18)"
  local efrac hfrac
  efrac=$(printf '%018d' 0)
...
# scripts/ethereum/susds.sh push (its own key).
cmd_push() {
  use_key HYDRATION_PRIVATE_KEY
  cmd_status || true
  ...
  cmd_status
  echo "Remember: run scripts/ethereum/susds.sh push for the Ethereum-side half."
}
