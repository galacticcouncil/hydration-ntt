#!/usr/bin/env bash
# WBTC — spoke leg: Hydration, burning mode. PRECOMPILE VARIANT (preferred):
...
#
# Run only AFTER the Ethereum hub leg (scripts/ethereum/wbtc.sh).
# Signs with HYDRATION_PRIVATE_KEY only (whitelisted Hydration deployer).
#
#   scripts/hydration/wbtc.sh preflight   # no txs: hub leg present, balance, whitelist note
#   scripts/hydration/wbtc.sh asset <id>  # no txs: derive+verify precompile ref from asset id
#   scripts/hydration/wbtc.sh deploy      # TX: manager+transceiver (burning), --skip-verify
#                                        #   (verify post-hoc via scripts/hydration/_verify.sh)
#   scripts/hydration/wbtc.sh limits      # no txs: write rate limits into deployment.json
#   scripts/hydration/wbtc.sh push        # TX Hydration: register Ethereum peer + limits
#
#   scripts/hydration/wbtc.sh minter      # no txs: print set_ntt_minter governance call
#   scripts/hydration/wbtc.sh status      # local vs on-chain drift
#
# Cross-link is two-sided: after 'push' here, also run scripts/ethereum/wbtc.sh
...
DEPLOYMENT="$HYD_ROOT/tokens/wbtc/deployment.json"
TOKEN_ADDR_FILE="$HYD_ROOT/tokens/wbtc/hydration-token.addr"

# 24h NTT rate limits, whole tokens. Ethereum side is 8-dec WBTC; Hydration
# side uses the runtime asset's decimals (read live from the precompile).
LIMIT_ETHEREUM_OUT="${LIMIT_ETHEREUM_OUT:-10}"   # not a stable — check spot and size before 'limits'
LIMIT_ETHEREUM_IN="${LIMIT_ETHEREUM_IN:-10}"
...
cmd_preflight() {
  ...
  check_cli
  [ -n "$(hub_manager)" ] \
    || { echo "ERROR: no Ethereum manager in $DEPLOYMENT — run scripts/ethereum/wbtc.sh first"; exit 1; }
...
cmd_asset() {
  local id="${1:?usage: wbtc.sh asset <asset-id>}"
  need_tools cast jq
  [ ! -s "$TOKEN_ADDR_FILE" ] \
  ...
  confirm "Use this asset as the WBTC representation?"
...
cmd_limits() {
  ...
  echo "Hydration asset decimals: $dec (Ethereum WBTC: 8)"
  local efrac hfrac
  efrac=$(printf '%08d' 0)
...
# scripts/ethereum/wbtc.sh push (its own key).
cmd_push() {
  use_key HYDRATION_PRIVATE_KEY
  cmd_status || true
  ...
  cmd_status
  echo "Remember: run scripts/ethereum/wbtc.sh push for the Ethereum-side half."
}
